$(function () {
    $("#jqGrid").jqGrid({
        ...
        datatype: "json",
        colModel: [
            {label: 'id', name: 'payId', index: 'payId', width: 50, key: true, hidden: true},
            {label: '订单号', name: 'outTradeNo', index: 'outTradeNo', width: 120},
            {label: '充值金额(元)', name: 'totalAmount', index: 'totalAmount', width: 60},
            {label: '用户名称', name: 'user', index: 'user', width: 60},
            {label: '用户账号', name: 'usercount', index: 'usercount', width: 60},
            {label: '订单状态', name: 'payType', index: 'payType', width: 80, formatter: orderStatusFormatter},
            // {label: '支付方式', name: 'payType', index: 'payType', width: 80,formatter:payTypeFormatter},
            {label: '备注信息', name: 'remark', index: 'remark', width: 10, hidden: true},
        ...
        success: function (result) {
            if (result.resultCode == 200) {
                ...
                for (i = 0; i < result.data.length; i++) {
                    itemString += result.data[i].goodsName + '  ' + ' 商品编号 ' + result.data[i].goodsId + ";<br>";
                }
...
function openExpressInfo(orderId) {
    ...
    $.ajax({
        type: 'GET',//方法类型
        url: '/admin/order-address/' + orderId,
        contentType: 'application/json',
        success: function (result) {
            if (result.resultCode == 200) {
                console.info(result);
                ...
                str += "<thead><tr><td>收货地址</td>" +
                    "<td>快递公司</td><td>快递号</td><td>操作</td></tr></thead><tbody>"
                var map = result.data;
                for(var key in result.data) {
                    str += '<tr>';
                    var tds = '<td>' + result.data[key].userAddress + '</td>';
                    tds += '<td>' + result.data[key].expressStatus + '</td>';
                    tds += '<td>' + result.data[key].expressNumber + '</td>';
                    if (result.data[key].sendType==0){
                        tds += "<td><button class='btn btn-info' onclick='orderEdit()'>发货</button></td>";
                    }else {
                        tds += '<td>修改物流信息</td>';
                    }
                    str += tds;
                ...
                // str +="<tr><td>aaa</td></tr>"
                str += '</tbody></table>';
                $('#userAddressInfo').html(str);
                // $("#userAddressInfo").html(result.data);
            } else {
                swal(result.message, {
                    icon: "error",
                });
